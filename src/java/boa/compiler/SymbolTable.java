...
public class SymbolTable {
	...
	static {
		...
		globalFunctions.addFunction("len", new BoaFunction(new BoaInt(), new BoaType[] { new BoaList(new BoaTypeVar("V")) }, "((long)${0}.size())"));
	...
	public static BoaType getType(final String id) {
		...
		if (id.startsWith("list of "))
			return new BoaList(getType(id.substring("list of ".length()).trim()));
	...
	private static void importLibs(final List<URL> urls) throws IOException {
		...
		final Class<?>[] builtinFuncs = {
			...
			boa.functions.refactoring.BoaRefactoringIntrinsics.class
		};
